...
Read in SD TOP: 
```{r,results="hide"}
  sd_pa<-read.csv("sensdoc-wearing-2023.csv") # 5575
```
...
```{r,echo=FALSE}
  si_plot<-ggplot(si[si$province %in% c("British Columbia","Saskatchewan","Quebec") & 
           ...
           annotate(geom="text", x=as.Date('2020-12-04'), y=10, label="Wave 2 data collection period",
                    color="grey30",size=2.75)

  si_plot
  
  ggsave(si_plot,file="figures//figure4.pdf",width=5.5,height=3.25)
  ggsave(si_plot,file="submission to IJE//figure4.pdf",width=175,height=95,unit="mm") # for IJBNPA
```
...
names(wave.labs)<-c("wave 1","wave 2")
  ...
  wear_plot<-ggplot(sd_valid,aes(x=wear)) + 
    ...
    labs(x="Minutes",
         y="Frequency",
         title="Wear time" #,
         #subtitle="Wave 1 (dark blue) vs. Wave 2 (light blue)"
         ) +
    geom_vline(xintercept=600,color="black",linetype="dashed") +
    facet_grid(vars(wave_id),
               labeller = labeller(wave_id=wave.labs)) +
    # scale_fill_manual(values=c("wave 1"="#0a9ffe", # green = "#81DA66"
    #                             "wave 2"="#fe7701"), # darker orange #fe7701 #ff8c09
    #                    labels=c("Wave 1","Wave 2")) + 
    scale_fill_grey(labels=c("Wave 1","Wave 2")) +
    scale_color_grey(labels=c("Wave 1","Wave 2"))+
  ...
  # titleMain=ggpubr::text_grob("Distribution of daily minutes of wear time and moderate-to-vigorous physical activity:\nWave 1 (2018-2019) vs. Wave 2 (2020-2021)\n",
  #                             size =10,face="bold")  
  titleY=ggpubr::text_grob("Frequency",
                            size =10,rot=90,face="bold")  
  
  figure1<-grid.arrange(wear_plot,mvpa_plot,ncol=2,
               #top=titleMain, # removing title for IJBNPA (titles should be in the text, not in the figure)
               left=titleY)
  
  ggsave(figure1,file="figures//figure1.pdf",width=6,height=4)
  ggsave(figure1,file="submission to IJE//figure1.pdf",width=175,height=100,unit="mm")
```
...
```{r,echo=FALSE,warning=FALSE}
  ...
  figure2<-ggplot(sd_valid[sd_valid$wave_id=="wave 1",],aes(x=doy3,y=mean_mvpa_sd,color=wave_id)) +
                          ...
                          scale_colour_manual(values=c("wave 1"="grey65",
                                                       "wave 2"="black"),
                                              labels=c("Wave 1 (2018-2019)","Wave 2 (2020-2021)")) +
                          geom_smooth(method=lm)+
                          geom_smooth(data=sd_valid[sd_valid$wave_id=="wave 2",],aes(x=doy3,y=mean_mvpa_sd),method=lm) +
                          theme_classic()+
                          labs(#title = "Daily minutes of moderate-to-vigorous physical activity by day of the year: ", # removing title for IJBNPA
                               #subtitle = "Wave 1 (2018-2019) vs. Wave 2 (2020-2021)",
                               y = "Daily minutes of moderate-to-vigorous physical activity",
                               x = "Date")+
  ...
  figure2
  ggsave(figure2,file="figure2.pdf",width=6,height=4)
  ggsave(figure2,file="submission to IJE//figure2.pdf",width=175,height=120,unit="mm")
```
...
em_figures<-grid.arrange(income_Plot.HandPick,
            ...
            heights=c(1),
            nrow=1)
ggsave(em_figures,file="figures//figure3.jpeg",width=175,height=85,units="mm")
ggsave(em_figures,file="figures//figure3.pdf",width=175,height=85,units="mm")
ggsave(em_figures,file="submission to IJE//figure3.pdf",width=175,height=85,units="mm")
```
...
```{r,echo=FALSE,result=FALSE,warning=FALSE}
  length(sd_pa[sd_pa$wave_id=="wave 2",]$interact_id) # missing 7 observations???
  length(sd_pa[is.na(sd_pa$stringencyIndex) & sd_pa$wave_id=="wave 2",]$interact_id)
```
